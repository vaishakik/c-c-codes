...
void get_transactions()
{
    ...
    for(i = 0; i < 5; i++)
    {
       ...
       // Depending on type input by user, get the values for appropriate structure members
...
void business()
{
    ...
    {
       if(p[i].delivery_type ==  1)
            home_delivery_charges = home_delivery_charges + p[i].charges;
       else if(p[i].delivery_type ==  2)
            web_charges = web_charges + p[i].charges;
       else
            voice_charges = voice_charges + p[i].charges;
    }
